...
const addProductToCart = productId => {
  ...
  //    key: productId
  //    data: productCount + 1
  const updatedProductCount = productCount ? productCount + 1 : 1;
  sessionStorage.setItem(productId, updatedProductCount.toString());
...
const getProductCountFromCart = productId => {
  ...
  return parseInt(sessionStorage.getItem(productId)) || 0;
};
...
const getAllProductsFromCart = () => {
  const productEntries = Object.entries(sessionStorage);

  const products = productEntries.flatMap(([productId, productCount]) => {
    productCount = parseInt(productCount);
    return productCount > 0 ? [{ productId, productCount }] : [];
  });
